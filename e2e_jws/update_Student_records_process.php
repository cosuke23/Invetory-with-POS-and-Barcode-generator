...
if(isset($_POST['btn_update_stud_info']))
	{
		...
		if(!empty($file_name_dp))
		{
			...
			$tbl = "audit_trail";
			$columns = ["date_at"=>$date_today,"time"=>$time_now,"username"=>$username,"action"=>"Update student info : ".$stud_no."","module"=>"Update Student Module"];
			$q_data = $database->insert($tbl,$columns);

			header("Location: e2e_student_records.php?success1=1&stud_no=$stud_no&lname=$lname&fname=$fname&mname=$mname");
