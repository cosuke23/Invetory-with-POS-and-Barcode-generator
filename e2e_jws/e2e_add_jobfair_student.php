            ...
            <div id="content">
               <div class="panel box-shadow-none content-header">
                  <div class="panel-body">
                    <div class="col-md-12">
                        <h1 class="animated fadeInLeft">ADD APPLICANT FOR JOB FAIR</h1>
                  ...
                  <div class="panel">
                    <div class="panel-heading">
                     ...
                     <ul id="tabs-demo4" class="nav nav-tabs nav-tabs-v3" role="tablist">
                      ...
                      <li role="presentation" class="active">
                        <a href="#tabs-demo4-area2" role="tab" id="tabs-demo4-2" data-toggle="tab" aria-expanded="false">CURRENT JOB FAIR COMPANY STUDENT APPLICANT</a>
                      </li>
                    </ul>
                    </div>
                    <div class="row">
                    ...
                    <div class="panel-body">
                        ...
                        if(isset($_GET['stud_no'])){
                          ...
                          $q_data_student_info = $database->query("SELECT a.*,b.* FROM student_info AS a INNER JOIN program_list as b WHERE a.stud_no = '$stud_no' AND a.program_id = b.program_id")->fetchAll();
                            foreach($q_data_student_info AS $q_data_si){
                                $lname = $q_data_si['lname'];
                                $mname = $q_data_si['fname'];
                                $fname = $q_data_si['mname'];
                                $program_code =  $q_data_si['program_code'];
                                $address = $q_data_si['address'];
                                $email = $q_data_si['email'];
                                $contact_no = $q_data_si['contact_no'];
                                $stud_dp = $q_data_si['stud_dp'];
                            }
                          }
                        ?>
                       ...
                       <?php
                       $q_data_em = $database->query("SELECT * FROM event_manager where status = 'active'")->fetchAll();
                       foreach($q_data_em AS $Data_em){
                          $event_id = $Data_em['event_id'];
                       }
                       ?>
                        <form action="e2e_admin_add_applicant_student_process.php" id="defaultForm" method="POST">
                          ...
                          <h5>COMPANY NAME</h5>
                           <div class="form-group has-feedback">
                              <input type="text" list="suggestion" class="form-control" name="comp_id" id="comp_id"/>
                              <?php
                              $query_nop_job_fair=$database->query("SELECT b.* FROM nop_job_fair AS a INNER JOIN company_info AS b INNER JOIN event_manager AS c WHERE a.comp_id = b.comp_id AND a.event_id = c.event_id AND c.status = 'Active'")->fetchAll();
                              print'<datalist id="suggestion">';      
                              foreach($query_nop_job_fair  AS $qemDAta){
                                $comp_id = $qemDAta["comp_id"];
                                $comp_name = $qemDAta["comp_name"];
								$job_fair_id = $qemDAta["job_fair_id"];
							...
							<input type="hidden" value="<?php echo $job_fair_id; ?>" name="job_fair_id"/>
                              <div class="col-md-3">
                            ...
                            <div class="col-md-4">
                                 <button type="submit" class="btn btn-ripple btn-raised btn-block btn-success"  name="btn_add_applicant_admin" id="btn_add_applicant_admin">
