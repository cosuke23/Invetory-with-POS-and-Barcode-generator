...
$columns = array(
  ...
  //  array( 'db' => '`a`.`acad_year_end`', 'dt' => 7, 'field' => 'acad_year_end' ),
   ...
   // array( 'db' => '`sa1`.`batch_id`', 'dt' => 10, 'field' => 'batch_id' ),
		array( 'db' => '`sa1`.`s1_timein`', 'dt' => 6, 'field' => 's1_timein',
		'formatter' => function($s1_timein) {
                        $in=$s1_timein;
						$F_s2_timein = date('h:i A',$in);
                        return  $F_s2_timein;
                    }
		),
		array( 'db' => '`sa1`.`s1_status`', 'dt' => 7, 'field' => 's1_status' ),
		array( 'db' => '`sa2`.`s2_timein`', 'dt' => 8, 'field' => 's2_timein',
		'formatter' => function($s2_timein) {
                        $in=$s2_timein;
						$F_s2_timein = date('h:i A',$in);
                        return  $F_s2_timein;
                    }
...
if(isset($_GET['semester']) && isset($_GET['acad_year_start']) && isset($_GET['program_id']) && isset($_GET['batch_id'])  && isset($_GET['event_id']) ){
...
if($batch_id == 0 && $program_id != 0){
    $extraWhere = "`a`.`acad_year_start` = '".$acad_year_start."' AND `a`.`program_id` = '".$program_id."' AND `a`.`semester` = '".$semester."' AND `sa1`.`event_id` = '".$event_id."'";
}elseif($program_id == 0 && $batch_id != 0){
    $extraWhere = "`a`.`acad_year_start` = '".$acad_year_start."' AND `a`.`semester` = '".$semester."' AND `sa1`.`event_id` = '".$event_id."' AND `sa1`.`batch_id` = '".$batch_id."'";
}elseif($batch_id == 0 && $program_id == 0){
    $extraWhere = "`sa1`.`acad_year_start` = '".$acad_year_start."' AND `sa1`.`semester` = '".$semester."' AND `sa1`.`event_id` = '".$event_id."'";
}else{
    $extraWhere = "`a`.`acad_year_start` = '".$acad_year_start."' AND `a`.`program_id` = '".$program_id."' AND `a`.`semester` = '".$semester."' AND `sa1`.`batch_id` = '".$batch_id."'";
  }
} 
echo json_encode(
