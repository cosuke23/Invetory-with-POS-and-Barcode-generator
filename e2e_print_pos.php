...
<table cellpadding="5" cellspacing="0" id="resultTable" border="1" width="100%">
	...
	<?php
                                ...
                                $from=$_POST['from'];
                                $to=$_POST['to'];
        $user_query = mysqli_query($conn,"SELECT * from sale where date_buyed>='$from' and date_buyed<='$to' ")or die(mysqli_error());
      }
...
while($item_data = mysqli_fetch_array($user_query)){
                          $username = $item_data['id'];
                          $itemname = $item_data['product'];
              ...
              $total=$item_data['product_total'];
                        ...
                        <br />
                        <?php
        $grand_query = mysqli_query($conn,"SELECT sum(product_total) as grand from sale where date_buyed>='$from' and date_buyed<='$to' ")or die(mysqli_error());
        $grand_data = mysqli_fetch_array($grand_query);
        $grand=$grand_data['grand'];
      
      ?>
                        <center>
                        Total Sale from <?php echo $from; ?> to <?php echo $to; ?> : <br />
