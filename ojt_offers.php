   ...
   <script src="assets/js/loadmore.js"></script>	
	...
	$(document).ready(function(){
		$(document).on('click','.show_more',function(){
			var ID = $(this).attr('id');
			var prgid = document.getElementById("prog_id").value;
			var srch = document.getElementById("search_query").value;
			$('.show_more').hide();
			$('.loding').show();
			$.ajax({
				type:'POST',
				url:'ajax_more_offers.php',
				data:'ojt_offers_id='+ID+'&program_id='+prgid+'&ojt_search='+srch,
      ...
      <section id="main-content">
          <section class="wrapper site-min-height">
							...
							<div class="weather-2-header row">
									<div class="">
										<div class="col-sm-12 col-xs-12">
											<p style="font-size:25px;padding-top:7px;">OJT OFFERINGS</p>
										</div>
										
									</div>
							</div><br>
							</div>
							<span style="font-size:17px;">Offers based on your course are shown here.</span></br>
							<span style="font-size:17px;">Click on the offer to view more information.</span></br><br>
							<form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>" method="POST">
							<div class="col-md-3 mb">
								<input type="text" class="form-control" name="ojt_search">
							</div>	
						...
						if(isset($_POST["btnsearch"])){
							...
							$query = mysqli_query($dbc, "SELECT * FROM ojt_offers where status='Available' and program_id='$stud_program_id' and (ojt_title LIKE '%$ojt_search%' OR ojt_desc LIKE '%$ojt_search%') ORDER BY 1 DESC LIMIT 10");
							$queryAll = mysqli_query($dbc,"SELECT COUNT(*) as num_rows FROM ojt_offers WHERE status='Available' and program_id='$stud_program_id' and (ojt_title LIKE '%$ojt_search%' OR ojt_desc LIKE '%$ojt_search%') ORDER BY ojt_offers_id DESC");
							$roww = mysqli_fetch_assoc($queryAll);
							$allRows = $roww['num_rows'];
						}else{
							$query = mysqli_query($dbc, "SELECT * FROM ojt_offers where status='Available' and program_id='$stud_program_id' ORDER BY 1 DESC LIMIT 10");
							$queryAll = mysqli_query($dbc,"SELECT COUNT(*) as num_rows FROM ojt_offers WHERE status='Available' and program_id='$stud_program_id' ORDER BY ojt_offers_id DESC");
							$roww = mysqli_fetch_assoc($queryAll);
							$allRows = $roww['num_rows'];
						}
						//number of rows
						...
						?>
							<input type="hidden" id="prog_id" value=<?php echo $stud_program_id; ?>>
							<input type="hidden" id="search_query" value=<?php echo $ojt_search; ?>>
						...
						echo '<span style="font-size:17px;">No results found for <b><i>'.$ojt_search.'.</i></b></span>';;
						}?>
