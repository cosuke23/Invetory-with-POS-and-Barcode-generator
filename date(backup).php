       ...
       popupWin.document.write('<html><body onload="window.print()">' + divToPrint.innerHTML + '</html>');
        popupWin.document.close();
                        ...
                        $date='08/15/2017';
                          $table = "dailyrecord"; 
                          $columns = "*";
                          $where= ["AND" => ["studno[=]" => $stud_id,"date[>]" => $date]];            
                          $where_em1 = ["studno[=]"=>$stud_id ];
                          
                          
              $q_item_info =$database->select($table,$columns,$where_em1);
              ?>
...
<br/> 
              ...
              <th class="text-center">REMARKS</th> 
                          ...
                          $date = $item_data['date'];
                          $time_in = $item_data['stime'];
                          $time_out = $item_data['etime'];
                          $total_minutes = $item_data['totalminutes'];
                          $total_hours = $item_data['totalhours'];
                         ...
                         ?></tbody>
                           ...
                           include('db.php');
                           $date='08/15/2017';
                          $query_idno = mysql_query("SELECT sum(totalhours)as total,sum(totalminutes)as totalm,date from dailyrecord where  studno='$stud_id'"); //error
