...
if(isset($_POST['btn-save']))
{
	...
	$address = mysqli_real_escape_string($dbc, $_POST['comp_address']);
	...
	if(mysqli_num_rows($result_select)>0)
	{
		...
		/*-------------------------------------query insert---------------------------------------*/
		$query_insert = "INSERT INTO endorsement(stud_no, adviser_id, comp_id, program_id, training_hours, date_created, submission_date, contact_person, position, contact_no, email, status, address) VALUES('$stud_no', '$adviser_id', '$comp_id', '$program_id', '$thours', '$date_created2', '$sub_date2', '$contact_person', '$position', '$contact_no', '$email', 'Active', '$address')";
		...
		if($result_insert == true)
		{
			...
			header("Location: http://sticaloocan.edu.ph/ojtassisti/mail/send_mail.php?endorsement=ok&title=$title&ad_fname=$ad_fname&ad_lname=$ad_lname&stud_fname=$stud_fname&stud_lname=$stud_lname&stud_mname=$stud_mname&stud_prog=$stud_prog&thours=$thours&comp_name=$comp_name&contact_person=$contact_person&contact_no=$contact_no&adviser_email=$adviser_email&admin_email=$admin_email");
			exit;
		}
		else
		{
