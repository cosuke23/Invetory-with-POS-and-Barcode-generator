...
require 'asset/connection/mysqli_dbconnection.php';
 $sem = $_GET['sem'];
 $acad_yr_start = $_GET['acad_yr_start'];
 $acad_yr_end = $_GET['acad_yr_end'];
 $status = $_GET['status'];
if($usertype==1)
{
...
if(isset($_GET['fileID']))
{
	...
	$query_update = "SELECT * FROM ojt_softcopy_files WHERE fileID='$file_id'";
	$result_update = mysqli_query($dbc, $query_update);
          if($result_update->num_rows > 0 )
            {   
              while ($row2 = mysqli_fetch_array($result_update))
              {
                $file_id_rd = $row2[0];
                $file_name = $row2[1];
                $file_desc = $row2[2];
            ...
            <div id="content">
               <div class="panel box-shadow-none content-header">
                  <div class="panel-body">
                    <div class="col-md-12">
                        <h1 class="animated fadeInLeft">UPDATE SEMESTERS</h1>
                        <p class="animated fadeInDown" style="padding-left:10px;">
                          Update Semesters in this module.
                        </p>
                    </div>
                  </div>
              </div>
              <div class="col-md-12 padding-0">
                <div class="col-md-12">
                  <div class="panel">
                    <div class="panel-heading"><h3>Update the <span><b><?php echo $status; ?></b></span> Semester</h3></div>
					<h4 style="padding-left:15px;">Note: All incoming registration will fall on <b>active</b> semester.</h4>
                   <br>
                    <div class="row">
                      <div class="col-md-12">
						<form  action="update_active_ongoing_sem_process.php" method="POST" enctype="multipart/form-data">
						<input type="hidden" name="status" value="<?php echo $status; ?>">
						<div class="form-group has-feedback" style="padding-left:15px;font-size:17px;color:#58666e;" ><span>Semester:<i style="color:red;">*</i></span><br>
							<select id="" name="sem" >
											<?php 
												
												if (isset($_GET['sem'])){
													if($_GET['sem'] == '1st Semester'){
														$selected1 = 'selected';
														$selected2 = '';
														$selected3 = '';
													}
													elseif($_GET['sem'] == '2nd Semester'){
														$selected1 = '';
														$selected2 = 'selected';
														$selected3 = '';
													}
													elseif($_GET['sem'] == 'Summer'){
														$selected1 = '';
														$selected2 = '';
														$selected3 = 'selected';
													}
											...
											<option <?php echo $selected1 ?> value="1st Semester">1st Semester</option>
											<option  <?php echo $selected2 ?> value="2nd Semester">2nd Semester</option>
											<option  <?php echo $selected3 ?> value="Summer">Summer</option>
							</select>
						</div>
						<div class="form-group has-feedback" style="padding-left:15px;font-size:17px;color:#58666e;" ><span>Academic Year:<i style="color:red;">*</i></span><br>
							<select id="" name="acad_year_start" >
											<?php 
											$yr_option1 = $acad_yr_start + 1;
											$yr_option1_end = $yr_option1 + 1;
											$yr_option2 = $acad_yr_start - 1;
											
											 echo '<option value="'.$yr_option2.'">'.$yr_option2.' - '.$acad_yr_start.'</option>';
											 echo '<option value="'.$acad_yr_start.'" selected>'.$acad_yr_start.' - '.$yr_option1.'</option>';
											 echo '<option value="'.$yr_option1.'">'.$yr_option1.' - '.$yr_option1_end.'</option>';
											?>
							</select>
							
						</div>
						<div style="padding-left:15px;font-size:17px;">
							<a href="update_semester_ay.php" class="btn btn-default">Cancel</a>
							&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
							<button type="submit" class="btn btn-primary" name="btnUpdateSem">Update</button>
						</div>
						</form>
                        </div>
                      </div>
