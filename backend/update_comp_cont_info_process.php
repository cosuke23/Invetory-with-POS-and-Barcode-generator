...
if(isset($_POST['btn-update']))
	...
	if(isset($_POST['btn-delete']))
	{
		...
		$fax_no =  mysqli_real_escape_string($dbc, trim($_POST['fax_no']));
		$email =  mysqli_real_escape_string($dbc, trim($_POST['email']));
		$username= mysqli_real_escape_string($dbc, trim($_POST['comp_username']));
		$date_notary2 = date('Y/m/d', strtotime($date_notary));
		$date_expiry2 = date('Y/m/d', strtotime($date_expiry));
//	$name = $_FILES['image']['tmp_name'];
		
	/**	$query_comp_info_delete = "UPDATE company_info
				SET comp_name = '$comp_name',comp_desc = '$comp_desc', address = '$address', city = '$city',
				 status = '$status', remarks = '$remarks', date_notary = '$date_notary2',
				date_expiry = '$date_expiry2',contact_person = '$contact_person', position = '$position', tel_no = '$tel_no',
				fax_no = '$fax_no', email = '$email' WHERE comp_id = '$comp_id'";
				$result_comp_info_delete = mysqli_query($dbc,$query_comp_info_delete);
				*/
				$query_comp_info_delete = "DELETE FROM company_info WHERE comp_id = '$comp_id'
				and status = '$status'  and username='$username'";
				$result_comp_info_delete = mysqli_query($dbc,$query_comp_info_delete);
				
			if($result_comp_info_delete)
			{
				echo "successfully deleted company" . "<br>";
				header("Location:Company_info.php?success3=2&comp_name=$comp_name");
			}else{
				echo "error";
			}
