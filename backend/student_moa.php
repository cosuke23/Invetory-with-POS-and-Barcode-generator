...
$num_row=mysqli_num_rows($result_student_pendingx);
          ...
          <!-- end: Left Menu -->
            ...
            <div id="content">
              ...
              <div class="col-md-12 padding-0">
                <div class="col-md-12">
                  <div class="panel">
                    <div class="panel-heading"><h3>Students with late M.O.A</h3></div>
 ...
 //$q_checkList = "SELECT * from student_checklist where deliverable_id=7 and remarks='Completed'";
  //            $q_check_res =$dbc->query($q_checkList);
     //         while($totalcheckcount = $q_check_res->fetch_assoc()){


              $q_totaldeliverables = "SELECT a.stud_no,a.fname,a.mname,a.lname,mr.date_note,mr.stud_no from moa_remarks as mr left join student_info as a  on a.stud_no=mr.stud_no left join student_checklist as sc on sc.stud_no=mr.stud_no   where  
              mr.date_note <='$date' and  sc.remarks='On process' group by a.stud_no";
              $q_totaldeliverables_res =$dbc->query($q_totaldeliverables);
         //   }
             
             //compute percentage 
              while($totaldeliverablescount = $q_totaldeliverables_res->fetch_assoc()){
                $stud_no=$totaldeliverablescount['stud_no'];
           
            
               ?>
                      <tr>        
                      <td><?php echo '<img src="../files/student_pics/'.$totaldeliverablescount['stud_no'].'.jpg" style="height:50px;width:60px;">';?></td> 
                                <td><?php echo $totaldeliverablescount['stud_no']; ?></td> 
                                <td><?php echo $totaldeliverablescount['lname']; ?></td> 
                                <td><?php echo $totaldeliverablescount['fname']; ?></td> 
                                <td><?php echo $totaldeliverablescount['mname']; ?></td> 
                                <td>
