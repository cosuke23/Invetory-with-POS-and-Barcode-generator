...
$joinQuery = "FROM `student_info` AS `a` JOIN `student_checklist` AS `c` JOIN `student_deliverables` AS `b` ON (`a`.`stud_no` =  `c`.`stud_no` AND `c`.`deliverable_id` = `b`.`deliverable_id` )";     
$extraWhere = "";
echo json_encode(
