...
//if "email" variable is filled out, send email
  ...
  if($q_check_res->num_rows > 0){
		...
		if($existingReq > $cur_time){
			  echo "A Reset Password link is already sent to your email. Please check your email inbox";
		}else{
			  $q_deleteExpired ="delete from forget_password_handler where user_id='$username'";
			  $q_deleteExpired_res = $con->query($q_deleteExpired);
		
			  $q_compemail = "select * from company_info where username='$username'";
			  $q_compemail_res = $con->query($q_compemail);
			  if($q_compemail_res->num_rows > 0){
			  $compemail= $q_compemail_res->fetch_assoc();
			  $email = $compemail['email'];
			...
			http://sticaloocan.edu.ph/ojtassisti/backend/comp_password_reset.php?link=".$encrypted_link."&username=".$username." 
			  
			<<This message is auto-generated and the link will expire in 24 hours. Please don't reply>>";
